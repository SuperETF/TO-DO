...
export default function PainLogModal({ memberId, onClose }: Props) {
  ...
  const handleSubmit = async () => {
    ...
    if (!error) {
      ...
      setActivity("");
      setNote("");
      toast.success("✅ 통증 기록이 저장되었습니다!");
    } else {
      toast.error("❌ 통증 기록 저장 중 오류 발생");
    }
      ...
      <div className="bg-white rounded-2xl w-full max-w-2xl h-[90vh] overflow-hidden shadow-xl flex flex-col">
        ...
        <div className="p-4 space-y-3">
          ...
          <button
            ...
            className={`w-full bg-teal-500 text-white py-3 rounded font-semibold hover:bg-teal-600 ${isSubmitting ? "opacity-60 cursor-not-allowed" : ""}`}
          >
